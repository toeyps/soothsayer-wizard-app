...
(รวมทั้งช่วงเวลา training), แก้บั๊กกราฟหาย 0 จุด, รองรับปี พ.ศ. อัตโนมัติ,
และปรับปรุง UI การเลือก sensor หลายจุด

### ✨ Features

- **Running Condition Filter** (เงื่อนไข "เครื่องกำลังทำงาน" ที่หน้า Build Model)
  - เลือกได้ว่าเงื่อนไขหลายข้อจะรวมกันแบบ **AND** (ต้องผ่านทุกข้อ) หรือ **OR** (ผ่านข้อใดข้อหนึ่งพอ)
  - แต่ละโมเดลเลือกได้เองว่าจะ **ใช้ค่า default ของ workspace** หรือ **ตั้งเงื่อนไขของตัวเอง**
    (Workspace / Custom) แทนที่จะบังคับใช้ค่าเดียวกันทุกโมเดลเหมือนเดิม
  - **ช่วงเวลา training (Time start/end)** ตั้งค่า default ระดับ workspace ได้แล้ว และรวมอยู่ใน
    ตัวเลือก Workspace/Custom เดียวกัน — ไม่ต้องตั้งทุกโมเดลจากศูนย์เหมือนเดิม
- **กราฟแดชบอร์ด**
  - workspace ที่เพิ่งเปิดครั้งแรก กราฟแสดง **6 เดือนล่าสุดของข้อมูล** เป็นค่าเริ่มต้น แทนที่จะโหลด
    ข้อมูลทั้งหมดทันที
  - ปุ่มเลือกช่วงเวลาด่วน (Y/M/W/D/H) อ้างอิงจาก **วันที่ล่าสุดในข้อมูลจริง** แทนวันที่ปัจจุบันของเครื่อง
    (แก้ปัญหากดแล้วไม่เจอข้อมูลถ้าข้อมูลเก่ากว่าวันนี้มาก)
  - เพิ่มป้าย **"Data: ‹start›–‹end›"** โชว์ช่วงข้อมูลทั้งหมดของ dataset ตลอดเวลา ไม่ว่าจะตั้ง filter ไว้แค่ไหน
- **รองรับปีพุทธศักราช (พ.ศ.) อัตโนมัติตอน import** — ตรวจจับและแปลงเป็น ค.ศ. ให้ทั้งแอป พร้อมแสดง
  warning จากตอน import ที่ไม่เคยโชว์ผู้ใช้มาก่อน (คอลัมน์ซ้ำ, timestamp ซ้ำ, ค่าที่ไม่ใช่ตัวเลข)
- **หน้า Build Model**
  - เปลี่ยนชื่อแผง "Data filter" เป็น **"Training scope"** ให้ชัดว่าคุมทั้งกราฟตัวอย่างและ training จริง
    พร้อมป้าย "Zoom = view only" กำกับที่ปุ่มซูมของกราฟ
  - ช่องเลือก **Predictor sensors** เปลี่ยนเป็น popup ค้นหา + จัดกลุ่มตาม Component + เลือกได้หลายตัวพร้อมกัน
  - ช่อง **Y sensor / X sensor / Criteria sensor** (Clustering) ใช้ popup แบบเดียวกัน แต่เลือกได้ทีละตัว
- **Scatter chart** — dropdown เลือกแกน X/Y แสดงคำอธิบาย sensor แทน tag เปล่าๆ ที่อ่านไม่รู้เรื่อง

### 🐛 Bug fixes

- กราฟเปลี่ยน aggregation หรือปรับ time filter แล้วข้อมูลหายไปทั้งหมด (0 จุด)
