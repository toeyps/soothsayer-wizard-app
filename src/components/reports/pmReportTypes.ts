...
// Shared between the PM page (dropdown + state default) and the report
// template (PDF label rendering). Stiffness is exposed to users as four
// discrete levels; the underlying λ is hidden everywhere user-facing.
// Order is loose → strict, matching the dropdown render order.
 ...
 *  the "Standard" label so the report never shows a bare number. */
