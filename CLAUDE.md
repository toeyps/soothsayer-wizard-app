...
- **⚠️ 2026-09-17: no UI path currently trains/saves a model.** The user had the toolbar's "Preview" button (and everything gated behind it — the Preview modal, the Confirm Save modal, `handleSaveModel`, `pickSavePath`, the save-plan derivation) removed from `PredictiveModelBuild.tsx` as unused, on top of an earlier pass the same day that already removed the toolbar's "Save Model"/"Report" buttons. `train_individual_model` / `train_relationship_model` / `train_clustering_model` (`src-tauri/src/lib.rs`) are unchanged and still grep-confirmed as the only callers of the GMM/LinearGAM training code, but nothing in `src/` invokes them anymore — they're live, tested (`cargo test --test predictive_model_tests`), and fully unreachable from the app. The Sub-models modal (multi-predictor Relationship comparison, opened via the `LayoutGrid` button on the Relationship chart) is unrelated and still works — it's preview-only, no save path. Don't assume "Save Model" exists anywhere in the UI without checking first; see `docs/PROJECT_HANDOVER.md`'s matching entry for the full account and the open question about whether to also remove the Rust side.
