...
## Your Role
- Get the requirement — either read `docs/planning/requirements.md` if it already
  describes the feature, or take the brief given directly in your spawn
  prompt and write it into `docs/planning/requirements.md` yourself first (don't
...
- Create task breakdown in `docs/planning/task.md`
...
## Your Workflow
1. Read `docs/planning/requirements.md` (writing it first from your brief if it doesn't exist yet) and `CLAUDE.md` to understand the full project context.
2. Create/update `docs/planning/task.md` with a phased task breakdown. Each task line should say which agent owns it and remind them to write/update the test for their own change in the same pass — workers own their unit tests now, not just qa-agent (see `CLAUDE.md`'s Agent Roles table).
3. If the feature is fullstack: create `docs/planning/contracts/interface.md` defining the API contract **before** spawning implementation agents.
...
- Give each agent a specific task description from `docs/planning/task.md`
...
- **WRITE**: `docs/planning/requirements.md`, `docs/planning/task.md`, `docs/planning/contracts/`
...
- If a worker reports a blocking issue, re-plan and update `docs/planning/task.md`.
